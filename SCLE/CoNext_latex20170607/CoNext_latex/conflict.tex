...
In Fig.~\ref{fig:Initial Graph}, the link set of  AP is $\mathbb{AP}=\{e_1,e_2,e_3,e_4$
...
\begin{proof}
    If a path of a unit-flow blocks all the links in \textbf{cut-set} $\mathbb{L}_{\Phi}$, \revtao{then no flow can use the links in  $\mathbb{L}_{\Phi}$ and thus no more flow can pass the cut $\Phi$}.
\end{proof}
...
The Set Cover Problem is usually an NP-hard problem with its complexity depending on the size of the \revtao{elements} (denoted by $n$). In our minimum SRLG Conflicting Link Set finding problem \revtao{$n=|\mathbb{L}_{\Phi}|$, that is, the edge number of cut set $\mathbb{L}_{\Phi}$}\del{, $n=|\mathbb{AP}|$, that is, the hop number of the AP}. As this paper's focus is not to improve the solution of the set cover problem, we apply the algorithm proposed in \cite{chvatal1979greedy} with the complexity $O(log(|\mathbb{L}_{\Phi}|))$  to find the minimum SRLG Conflicting Link Set. Usually, even for a large scale network, the  AP with the shortest path does not have a large number of hops. Therefore, the cost to calculate the minimum SRLG Conflicting Link Set through the set cover is not large.
...
According to the graph style of SRLG, there are two categories\revtao{\cite{datta2008graph}}:  star-style and none star-style. For star-style SRLG, all the links start from the same node or ends at the same node. For example, in Fig.\ref{fig:MinCutStarGraph}, $e_1$ and $e_9$ start from the \note{Start-style or star style?} the same node $s$, $\mathbb{R}_{r_1}$ is a star-style SRLG. \del{Similarly, $\mathbb{R}_{r_4}$ and $\mathbb{R}_{r_5}$ are star-style SRLGs.} For none star-style SRLG, not all links in the SRLG starting from the same node or ending at the same node. In Fig.\ref{fig:MinCutStarGraph}, $\mathbb{R}_{r_2}$, $\mathbb{R}_{r_3}$,$\mathbb{R}_{r_4}$ and $\mathbb{R}_{r_5}$ are none star-style SRLGs.
