...
\begin{figure}[htbp]
  ...
  \caption{Bhandari算法示例}\label{fig:BhandariAlg}
\vspace{\baselineskip}
\end{figure}
...
\begin{algorithmic}[1]
    ...
    \FOR{$i\leftarrow\ 2\ to\ M$}
        ...
        \STATE{用反向边替换在用原图中所有$P_x(x<i)$路径的每条链路}
        \STATE{寻找从节点$s$到节点$d$的最短路径$P_i$}
        \STATE{删除所有重叠链接，以获得我不相交的路径$P_x$，其中$x\leq i$}
    \ENDFOR
\end{algorithmic}
}
\par}
...
\begin{figure}[htbp]
\centering
\subfigure[Physical topology with conduit]{\includegraphics[width=2.2 in]{figures/PhysicalGraph}
}
\subfigure[Network Graph]{\includegraphics[width=1.8 in]{figures/VirtualGraph}
}
