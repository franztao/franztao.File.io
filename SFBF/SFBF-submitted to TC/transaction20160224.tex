...
%\vspace{-0.6in}
\begin{abstract}
Bloom filters (BF) allow membership queries over sets with allowable error rate. It is widely used in databases, networks and distributed systems. \rev{Despite the fact that BF can work well to represent a static data set where BF parameters are set to achieve a low false positive rate based on the knowledge of the data set size, when there are dynamic data arrivals thus the constant data set change, it would lead to high false positive errors in BF.} To make BF scalable, we propose a Scalable and Flexible Bloom filter (SFBF) to control the false positive rate at a low level even when the \rev{ever-increasing data arrives and the total number of data items is unpredictable}. Specifically, we propose a novel algorithm to adaptively generate hash functions and a light-weight SFBF query algorithm that takes advantage of the features of our proposed hash functions to \rev{provide quick BF query}. Our performance results demonstrate that SFBF offers very good performance (low false positive rate and low query time) in the presence of \rev{dynamic data arrival}, even when the data set becomes very large.
\end{abstract}

%\note{If this is not a pure BF paper, depending on where to submit the paper, you need to give applications as motivation. Pure BF paper may need to be submitted to some theory or Math journal.  Dear sister, I write this paper as a pure BF paper. I think BF is a very useful data structure.}
...
%\note{Why do you need to determine duplication here? This application appears suddenly without clear meaning for it. Dear sister, I have modified the paper by not using duplication}
...
\begin{figure}[!h]
\centering
\subfigure[Insertion operation]{\includegraphics[width=1.67 in]{fig/BFinsert}\label{fig:BFinsert}}
\subfigure[False positive]{\includegraphics[width=1.67 in]{fig/BFfalse}\label{fig:BFfalse}}
...
Although a basic Bloom filter can work well to represent a static set whose size is known before the design and deployment, it may lead to a high false positive rate with dynamic data arrivals. When the number of data items represented by a BF $n > n_0$, the false rate would be larger than $f_0$, and even approach 1. This would make every new coming data falsely reported to \rev{be in the set while it is not}, %\note{what do you mean duplicated here? I made modification} 
and render the BF useless. Simply setting $m$ to a large value would help to accommodate more data, but it would lead to high storage and communication overhead  as Bloom filters are usually applied as messages to exchange among distributed nodes in a network.

In addition, without the prior knowledge of \rev{the size of the data set}, it is also difficult to properly determine the value of $m$.
...
Instead of providing a static expanding solution to handle the scalability problem, the BF design should take flexible and dynamic expanding rate into consideration according to the \rev{dynamic set changes in networks and distributed systems.}

%\note{Why dynamic network environment? You did not mention this is for network mainly. Dear sister, I made a small modification accordingly.}
...
%\note{please pay attention the difference between "trade off" and "tradeoff", Dear sister, I remember.}
...
%\note{Again, you use the word "network environment". You did not give any application that is relate to the network environment when you preseting the problem earlier.}
...
To make the Bloom filter scalable with the size of the data set, the key technique taken by SFBF is to add a new BF vector upon need. The length of the BF vector is determined by the expanding speed instead of being fixed.
 %\note{You put "according to the dynamic network environment". You did not mention any network applications related to your BF.}
...
%\note{Don't always use the word "obviously". Don't always put a long sentence an then put 's after it. Are you really trying to drive me and readers crazy? Dear sister, I will change}
...
\begin{figure}[!h]
\centering
\subfigure[Left logic shift]{\includegraphics[width=1.6 in]{fig/Left_logical_shift}
\label{fig:Left logic shift}}
\subfigure[Right logic shift]{\includegraphics[width=1.6 in]{fig/right_logical_shift}
...
%\note{You mention duplication detection in the above paragraph, but you did not introduce what is duplication detection earlier. Dear sister, I made a modification}
...
%\note{For each of the above traces, what kind of application do you use the BF for? This helps to explain the Application of SFBF. BTW, this will also impact the journal you can submit to. Dear sister, I use bloom filter as the data structure, all the traces are the data. I use the real data set to evaluate the performance of the data structure.}

For the first trace MAWI, we extract the Source IP address (32bit) from the MAWI trace  as the key  to insert into and query against the SFBF. \rev{For the synthetic trace, the generated data item is directly used as the key to operate against the SFBF. }
For the last two  traces, we apply MurMurhash's hash to the URL string to generate the 32 bit URL fingerprint, and use the fingerprint as the key \rev{(the data item)} to operate against the SFBF. %\note{Does this hash have the shifting feature you introduced? Dear sister, the fingerprint is the data item to operate against the SFBF}
...
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
    ...
    \begin{tabular}{|c|c|c|}
        ...
        \specialcell[c]{MAWI 1} & \specialcell[c]{2015-01-01\\ {[}14:00:01-14:00:15{]}} & 58066726  \\\hline
...
Utilizing above traces, we perform a set of experiments to evaluate the performance of SFBF on a LENOVO workstation  equipped with Intel(R) Core(TM)i5-5200U CPU(2.20GHz) and 8.00GB RAM. Each experiment dedicates to a specific SFBF, which is associated with randomly selected $k$ base hash generation matrices of dimension $32 \times 32$, i.e., the number of columns is $w=32$ and the number of rows is $R=32$.  \rev{As the IP address in MAWI trace is  32bit, the URL fingerprint in traces Calgary-HTTP and NASA-HTTP is 32 bit, and data in the Synthetic trace are generated within the range from 0 to $2^{32}-1$, we set the No.column in the base hash generation matrices to be 32}. The length and the capacity of the \rev{baseline BF vector} in SFBF are
set to $m_0 = 1024$ and $n_0 = 64$, respectively. The number of hash functions is set to $k = 6$.


Each experiment includes two steps.

In the first step, following the insertion operation in Algorithm \ref{alg:Insertion Operations}, we use an SFBF \rev{which consisting of multiple BF vectors} to represent  \rev{the first 30,000 data items in} the data trace \rev{with each data item  arriving sequently}.  $k$ hash functions are applied to each data item to set $k$ locations of the active BF vector to 1. \rev{Starting with the small baseline BF vector, when the data expands, SFBF expands} and a new BF vector is created and added into SFBF if necessary with the filter length determined by the expanding speed $\lambda$. %\note{You have static data. How to determine the speed? Dear sister, the expanding speed is another parameter utilized in our BF. In this paper, we just evaluate performance using different speed} 
After all data items are represented by SFBF, the space usage of SFBF is known.

In the second step, in order to evaluate the performance, after all data items in a data set have been represented by SFBF, we use another 150,000 data items in the traces to determine the false positive rates of SFBFs. 
%\note{Where do you get another items if all items have been in the filter? Unclear. Dear sister, Now I modified} 
 We run the querying Algorithm \ref{alg:Low cost query algorithm} using the same $k$ hash generation matrices in the insertion step. If the querying process returns true for a data item not in the set, it is counted as a filter false positive. The false positive rate for SFBF \rev{is the fraction of the false positive in 150,000 queries.} We also insert a timer to measure the querying time.

%\note{Did you mention there is no false negative in BF earlier? People without BF may not know this. Dear sister, I add it in the introduction}

Besides SFBF, for performance comparison, we implement DBF proposed in~\cite{guo2006theory}  \rev{which expands
the Bloom filter by adding a fixed-size BF vector each time
as the set size increases.}  %\note{How does DBF work? Did you describe somewhere before? If not, you need to give brief introduction here. There are basics, and you are not beginning researcher. Dear sister, I have added the description of DBF} 
For each parameter combination, we repeat our experiments 100 times and take their average.

Moreover, as SFBF is designed to represent a dynamic data set, to evaluate the performance of SFBF under different expanding speed, different expanding speed $\lambda $ are \rev{simulated to control the SFBF's expanding process}. 
%\note{You have static traces. No one understand why you need to generate expanding speed, and how they are related to your trace data and practical applications. Why can't you explain anything clearly? Dear sister, I added little. Actually, I do not know why it is not clear}
\begin{itemize}
...
%\note{Why do you still call it experiment setup? Also, why not putting some introduction at the beginning of the section? Dear sister, I made a modification}

\rev{To evaluate the performance of SFBF, }four performance metrics are applied to evaluate the proposed SFBF:
\begin{itemize}
...
\begin{figure*}[!htb]
\center
\subfigure[Synthetic 1]{\includegraphics[width=2.35in]{franztao20160111/test1/Extensionround/test1_virtualdata}
\label{fig:Extension round test1 virtualdata.}}
\subfigure[MAWI 1]{\includegraphics[width=2.35in]{franztao20160111/test1/Extensionround/test1_actualdata_IP_1}
\label{fig:Extension round test1_actualdataIP_1.}}
\subfigure[MAWI 2]{\includegraphics[width=2.35in]{franztao20160111/test1/Extensionround/test1_actualdata_IP_2}
\label{fig:Extension round test1_actualdataIP_2.}}\\
\subfigure[Calgary-HTTP]{\includegraphics[width=2.35in]{franztao20160111/test1/Extensionround/test1_actualdata_webcache_1}
\label{fig:Extension round test1 actualdata webcache 1.}}
\subfigure[NASA-HTTP]{\includegraphics[width=2.35in]{franztao20160111/test1/Extensionround/test1_actualdata_webcache_2}
...
For the five implemented traces, at the same BF's parameter setting, the extension round under Synthetic trace is largest among all other traces. All data items in the Synthetic trace are randomly generated which have a high probability to be unique, but many data items in other traces are duplicated (duplicated source IP address in  MAWI trace, and duplicated URL in Calgary-HTTP and NASA-HTTP traces). Therefore, even though the \rev{total number of data items} to represent are the same, the total unique data items in the Synthetic trace are larger than those in other traces, which results in \rev{the larger extension round and higher false positive rate (in Fig.\ref{fig:False positive rate.}).}

\rev{As shown in Fig.\ref{fig:Extension round.},} different from our SFBF, the extension procedures under DBF depends on the size of the data set, and the number of extension rounds \rev{is more like a}  linear function of the size of data set.


%\note{Don't put "And" at the beginning of a sentence, it is not formal in technical article. I remember mentioning this many times in the past. I really hope you can remember things, and don't make the same mistakes over an over.}


%\note{Why not analyzing the round result here? Dear sister, I have made a modification. }
...
\begin{figure*}[!htb]
\center
\subfigure[Synthetic 1]{\includegraphics[width=2.35in]{franztao20160111/test1/Falsepositiverate/test1_virtualdata}
\label{fig:Falsepositiverate test1 virtualdata.}}
\subfigure[MAWI 1]{\includegraphics[width=2.35in]{franztao20160111/test1/Falsepositiverate/test1_actualdata_IP_1}
\label{fig:Falsepositiverate test1_actualdataIP_1.}}
\subfigure[MAWI 2]{\includegraphics[width=2.35in]{franztao20160111/test1/Falsepositiverate/test1_actualdata_IP_2}
\label{fig:Falsepositiverate test1_actualdataIP_2.}}\\
\subfigure[Calgary-HTTP]{\includegraphics[width=2.35in]{franztao20160111/test1/Falsepositiverate/test1_actualdata_webcache_1}
\label{fig:Falsepositiverate test1 actualdata webcache 1.}}
\subfigure[NASA-HTTP]{\includegraphics[width=2.35in]{franztao20160111/test1/Falsepositiverate/test1_actualdata_webcache_2}
...
%  \label{tab:3}%
%\end{table*}
To illustrate the false positive rates under different Bloom Filter schemes, besides DBF and SFBF, Fig.\ref{fig:False positive rate.} also shows the curve of basic Bloom filter (denoted as BasicBF). From Fig.\ref{fig:False positive rate.}, with the data set expanding to 30,000, the false positive rate increases under all BF schemes. With the continuous increase of \rev{the set size} $n$, the false positive rate of BasicBF approaches 1 rapidly, while our SFBF with all various expanding speed $\lambda$ can well control the false positive rate to remain at very low level.

The small figure in Fig.\ref{fig:False positive rate.} remove the curve of BasicBF to clearly compare DBF and SFBF. \rev{From the small figure, we can observe that the larger the expanding speed $\lambda$, as a result of smaller extension round needed to represent the dynamic data set, the smaller the false positive ratio of our SFBF. }

%\note{your curves show the false positive of Basic BF reaches 1 from the beginning, not after 30,000. Dear sister, the fig shows that Basic BF reaches 1 rapidly, not from the beginning but very quickly}

When the data set expands to $n = 30,000$, which is 388 times that of $n_0= 64$, the false positive rates of DBF reach 30.59\% (for the Synthetic trace in Fig.\ref{fig:Falsepositiverate test1 virtualdata.}),
...
%\note{Don't need to put the before plural nouns, unless you have a adjective word or sentence to describe it. Dear sister, I remember}



%\note{You did not analyze other impacts, and the impact of lambda. Dear sister, I added the impact analysis}

%Although SBF has the minimum false positive rate, it costs larger amount of space compared to SFBF, as we will see in Fig.\ref{fig:Size of filters.}.
...
\begin{figure*}[!htb]
\center
\subfigure[Synthetic 1]{\includegraphics[width=2.35in]{franztao20160111/test1/QueryingCPUtimewithshifting/test1_virtualdata}
\label{fig:QueryingCPUtimewithshifting test1 virtualdata.}}
\subfigure[MAWI 1]{\includegraphics[width=2.35in]{franztao20160111/test1/QueryingCPUtimewithshifting/test1_actualdata_IP_1}
\label{fig:QueryingCPUtimewithshifting test1_actualdataIP_1.}}
\subfigure[MAWI 2]{\includegraphics[width=2.35in]{franztao20160111/test1/QueryingCPUtimewithshifting/test1_actualdata_IP_2}
\label{fig:QueryingCPUtimewithshifting test1_actualdataIP_2.}}\\
\subfigure[Calgary-HTTP]{\includegraphics[width=2.35in]{franztao20160111/test1/QueryingCPUtimewithshifting/test1_actualdata_webcache_1}
\label{fig:QueryingCPUtimewithshifting test1 actualdata webcache 1.}}
\subfigure[NASA-HTTP]{\includegraphics[width=2.35in]{franztao20160111/test1/QueryingCPUtimewithshifting/test1_actualdata_webcache_2}
...
\begin{figure*}[!htb]
\center
\subfigure[Synthetic 1]{\includegraphics[width=2.35in]{franztao20160111/test1/QueryingCPUtimewithoutshifting/test1_virtualdata}
\label{fig:QueryingCPUtimewithoutshifting test1 virtualdata.}}
\subfigure[MAWI 1]{\includegraphics[width=2.35in]{franztao20160111/test1/QueryingCPUtimewithoutshifting/test1_actualdata_IP_1}
\label{fig:QueryingCPUtimewithoutshifting test1_actualdataIP_1.}}
\subfigure[MAWI 2]{\includegraphics[width=2.35in]{franztao20160111/test1/QueryingCPUtimewithoutshifting/test1_actualdata_IP_2}
\label{fig:QueryingCPUtimewithoutshifting test1_actualdataIP_2.}}\\
\subfigure[Calgary-HTTP]{\includegraphics[width=2.35in]{franztao20160111/test1/QueryingCPUtimewithoutshifting/test1_actualdata_webcache_1}
\label{fig:QueryingCPUtimewithoutshifting test1 actualdata webcache 1.}}
\subfigure[NASA-HTTP]{\includegraphics[width=2.35in]{franztao20160111/test1/QueryingCPUtimewithoutshifting/test1_actualdata_webcache_2}
...
%\note{You said you had inserted all trace items into BF earlier. Where did you get the "another"? Dear sister, I made a modification}
...
%\note{If the magnitude of the values have big differences, you may use log. For example, in Fig 10, you may use semilog function to make the Y axis log value, rather than not able to show anything for shifting operations. As our SFBF result is too low, although we have transferred the figure to log, our SFBF are still very low and nearly same to the current fig.}
 ...
 Comparing the Shifting scheme in Fig.\ref{fig:Querying CPU time with shifting.}  with the Direct scheme in Fig.\ref{fig:Querying CPU time without shifting.}, obviously, our Shift scheme proposed is very efficient and effective in largely reducing the query CPU time. Take the trace MAWI1 for example, when the data set expands to $n = 30,000$, the average querying CPU time under SFBF-$\lambda_1$-Shift, SFBF-$\lambda_2$-Shift, SFBF-$\lambda_3$-Shift, SFBF-$\lambda_4$-Shift, SFBF-$\lambda_5$-Shift, and are only 1.8\%, 2.0\%, 1.4\%, 2.2\% and 1.7\% of those under SFBF-$\lambda_1$-Direct, SFBF-$\lambda_2$-Direct, SFBF-$\lambda_3$-Direct, SFBF-$\lambda_4$-Direct and SFBF-$\lambda_5$-Direct.
...
The query time of DBF consists of hash function computation time and comparison time while the later depends on the total number of extension rounds. The query CPU time of SFBF consists of the hash function computation time, comparison time, and shifting time, among which the later two depend on the total number of extension rounds.
 ...
 Compared to DBF, our SFBF can well control the extension round even when the data set is expanded to a very large size, as shown in Fig.\ref{fig:Extension round.}, therefore the increase of querying time in DBF is much faster than SFBF when the set expands, as shown in Fig.\ref{fig:Querying CPU time with shifting.}. When the data set expands to $n = 30,000$, DBF needs much more CPU time than those of SFBF, as shown in Fig.\ref{fig:Querying CPU time with shifting.}. Take the trace MAWI1 as an example, the average query time of DBF is 0.027 ms, while the time of SFBF-$\lambda_1$-Shift, SFBF-$\lambda_2$-Shift, SFBF-$\lambda_3$-Shift, SFBF-$\lambda_4$-Shift, and SFBF-$\lambda_5$-Shift are $0.1733*10^{-3}$ ms, $0.1743*10^{-3}$ ms, $0.1856*10^{-3}$ ms, $0.2689*10^{-3}$ and $0.1989*10^{-3}$ ms, respectively. \rev{This also   demonstrates that our shifting operation is  very
...
For the different traces, the query time under the Synthetic trace are much larger than other traces. \rev{To query a data item not presented by the SFBF, all the BF vectors should be checked unless a false positive happens.} \rev{More BF vectors means large query time.} One the one hand the data items have a high probability to be  unique to each other, thus the  Synthetic trace needs more BF vectors to represent the data set compared with other traces.    On the other hand, as the data in other traces have more duplicated items than that in Synthetic, the query hit ratios  in other traces are larger than that in Synthetic \rev{when query the another 150,000 data items.} \rev{When a query hit happens, this query does not need to check all the BF vectors, thus large query hit ratios means smaller query time when same number of queries happens.}

%\note{These two reasons are conflicting. Dear sister, The first reason seems to indicate duplicated items won't occupied more space in BF, while the second implies they have multiple places inserted and easy to be found.}
...
\begin{figure*}[!htb]
\center
\subfigure[Synthetic 1]{\includegraphics[width=2.35in]{franztao20160111/test1/Spacesizeoffilters/test1_virtualdata}
\label{fig:Spacesizeoffilters test1 virtualdata.}}
\subfigure[MAWI 1]{\includegraphics[width=2.35in]{franztao20160111/test1/Spacesizeoffilters/test1_actualdata_IP_1}
\label{fig:Spacesizeoffilters test1_actualdataIP_1.}}
\subfigure[MAWI 2]{\includegraphics[width=2.35in]{franztao20160111/test1/Spacesizeoffilters/test1_actualdata_IP_2}
\label{fig:Spacesizeoffilters test1_actualdataIP_2.}}\\
\subfigure[Calgary-HTTP]{\includegraphics[width=2.35in]{franztao20160111/test1/Spacesizeoffilters/test1_actualdata_webcache_1}
\label{fig:Spacesizeoffilters test1 actualdata webcache 1.}}
\subfigure[NASA-HTTP]{\includegraphics[width=2.35in]{franztao20160111/test1/Spacesizeoffilters/test1_actualdata_webcache_2}
...
%\note{SFBF size, specially at lamda2, is much larger than DBF. Why do you call it the same? Dear sister, I delete the description on  lamda 2}
