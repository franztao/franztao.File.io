...
\begin{itemize}
  ...
  \item \textbf{UMass \cite{UMass}:This trace contains trace data about user requests for specific YouTube content.}
  ...
  \item \textbf{TKN \cite{TKN}: This trace contains Trace Files of standard Movies such as Jurassic Park I}.
...
\textbf{For the UMass traces, we concatenate Video ID and Content server IP into a string, then apply MurMurhash's hash to the string to generate the 32 bit URL fingerprint,which is used as the key to operate against the SFBF.
For the last trace, we concatenate every four integer data of trace files into a string,then then apply MurMurhash's hash to the string to generate the 32 bit fingerprint,which is used as the key to operate against the SFBF.}
...
\center
  ...
  \includegraphics[width=3in]{franztao20161209/p2_ExtensionRound}\\
...
As shown in Fig.\ref{fig:Extension round.}, different from our SFBF, the extension procedures under DBF depends on the size of the data set, and the number of extension rounds increases about approximately linear with the size of data set.
...
When the data set expands to $n = $\DataSetSize, which is 2907 times that of $n_0= 64$, the false positive rate of DBF reaches 93.24\% (for the Synthetic trace in Fig.\ref{fig:False positive rate.}(a)),
...
\begin{figure}[!h]
\center
\includegraphics[width=3in]{franztao20161209/p5_QueryingCPUtimewithshifting}\\
...
\begin{figure}[!h]
\center
\includegraphics[width=3in]{franztao20161209/p4_QueryingCPUtimewithoutshifting}\\
...
\begin{figure}[!h]
\center
\includegraphics[width=3in]{franztao20161209/p3_SpaceSizeFilters}\\
...
Take the trace MAWI as an example, when the data set expands to \DataSetSize, even though the space usage under DBF, SFBF-$\lambda_1$,SFBF-$\lambda_2$,  SFBF-$\lambda_3$, SFBF-$\lambda_4$ and SFBF-$\lambda_5$ is close, the false positive rates of SFBF-$\lambda_1$,SFBF-$\lambda_2$,  SFBF-$\lambda_3$, SFBF-$\lambda_4$ and SFBF-$\lambda_5$ are only 1.38\%,  0.68\%, 2.57\%, 69.49\% and 36.30\% of that of DBF, and the query time of SFBF-$\lambda_1$-Shift,  SFBF-$\lambda_2$-Shift,SFBF-$\lambda_3$-Shift, SFBF-$\lambda_4$-Shift, SFBF-$\lambda_5$-Shift are 0.58\%, 0.57\%, 0.75\%, 7.09\% and 4.02\% of that of DBF, as can be seen from Fig.\ref{fig:False positive rate.} and Fig.\ref{fig:Querying CPU time with shifting.}.
