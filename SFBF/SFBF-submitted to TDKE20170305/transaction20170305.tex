...
\author{Kun Xie$^{1,2}$, Heng Tao$^2$, Xin Wang$^2$, Yinghua Min$^3$ \emph{Fellow, IEEE}, \\ Jigang Wen$^3$, Dafang Zhang$^2$, Keqin Li$^4$ \emph{Fellow, IEEE},  Gaogang Xie$^3$\\
...
$^4$ Department of Computer Science, State University of New York,  USA\\
%$^3$ School of Information Science and Engineering, Hunan University, Changsha 410082, China\\
kun.xie@stonybrook.edu, franztaoheng@gmail.com, x.wang@stonybrook.edu,  min@ict.ac.cn, dfzhang@hnu.edu.cn, wenjigang@ict.ac.cn, lik@newpaltz.edu, xie@ict.ac.cn}
\maketitle
\vspace{-3em}
%\vspace{-0.6in}
\begin{abstract}
Bloom filters (BF) allow membership queries over sets with allowable error rate. It is widely used in databases, networks and distributed systems. Despite that BF can work well to represent a static data set by optimally setting the BF parameters to achieve a low false positive rate based on the data set size, when there are dynamic data arrivals thus constant data set changes, it would lead to high false positive errors in BF. To make BF scalable, we propose a Scalable and Flexible Bloom filter (SFBF) to \del{control the false positive rate  at a low level } \rev{efficiently represent the data set with low false positive rate} even when there are ever-increasing data arrivals and the size of the data set is unpredictable. Specifically, we propose a novel algorithm to adaptively generate \rev{independent} hash functions and a light-weight SFBF query algorithm that takes advantage of the features of our proposed hash functions to provide quick BF query. Our performance results demonstrate that SFBF offers very good performance (low false positive rate and low query time) in the presence of dynamic data arrivals, even when the data set becomes very large.
\end{abstract}
...
\rev{As our SFBF is designed based on basic bloom filter, a problem with our SFBF is that we can not delete an element from its  corresponding Bloom filter vector by changing the corresponding 1s to 0s. %Therefore,  SFBF can hardly support element deletion when it is designed based on basic bloom filter.
...
%\end{equation}
%
%let k hash functions is independent($u_1 \bigwedge Q_{r \times w}^1 \oplus u_2\bigwedge Q_{r \times w}^2 \oplus \ldots \oplus u_k\bigwedge Q_{r \times w}^k$. $Q_{r \times w}= ZeroMatrix\ if\ only\ if\ r1=r2=\ldots=r_k=0$). we can set the matrix $Q_{r \times w}^k $ as below, 第1到第k行元素都设置成只有第k列元素为1的向量，从第k+1 行到第R 行元素都随机设置。
