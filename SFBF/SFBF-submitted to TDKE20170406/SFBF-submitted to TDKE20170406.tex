...
%\end{equation}
%
%let k hash functions is independent($u_1 \bigwedge Q_{r \times w}^1 \oplus u_2\bigwedge Q_{r \times w}^2 \oplus \ldots \oplus u_k\bigwedge Q_{r \times w}^k$. $Q_{r \times w}= ZeroMatrix\ if\ only\ if\ r1=r2=\ldots=r_k=0$). we can set the matrix $Q_{r \times w}^k $ as below, 第1到第k行元素都设置成只有第k 列元素为1的向量，从第k+1 行到第R 行元素都随机设置。
